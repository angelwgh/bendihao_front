...
define(function () {
    ...
    authorizationSystem.run(['$http', '$rootScope', 'authorize', 'hbBasicData', 'HBInterceptor',
        function ($http, $rootScope, authorize, hbBasicData, HBInterceptor) {
            $rootScope.showApp_$$ = false;
            $http.get('../FxbManager/userController/userInfo')
                .then(function (data) {
                    /*if (data.data.status === true) {
                        if (data.data.info) {
                            if (data.data.info.userId === null || data.data.info.userId === '') {
                                window.location.href = '/login/login.html';
                            } else {
                                $rootScope.showApp_$$ = true;
                                $rootScope.$$userInfo = data.data.info;
                                hbBasicData.imageSourceConfig.uploadImageUrl = data.data.info['resourceServicePath'];
                            }
                        }
                    } else {
                        $rootScope.showApp_$$ = false;
                    ...
                    if (data.data.state === 1) {
                        if (data.data.jsonBody) {
                            if (data.data.jsonBody.userId === null || data.data.jsonBody.userId === '') {
                                ...
                                hbBasicData.token = data.data.jsonBody['hbBasicData'];
                                hbBasicData.imageSourceConfig.uploadImageUrl = data.data.jsonBody['resourceServicePath'];
                                //hbBasicData.imageSourceConfig.uploadImageUrl = data.data.jsonBody.info['resourceServicePath'];
                            }
                        }
                    } else {
                        $rootScope.showApp_$$ = false;
